      ...
      <div className="bg-white dark:bg-slate-800 border-l-4 border-gov-navy shadow-sm rounded-md p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6 border border-slate-200 dark:border-slate-700">
        <div className="space-y-2 text-center sm:text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold border border-slate-200 dark:border-slate-600">
            <ShieldCheck className="w-4 h-4 text-gov-navy dark:text-blue-400" /> {t('digilockerVault', 'Official DigiLocker-Style Vault')}
          </div>
          <h1 className="text-3xl font-bold font-heading tracking-tight text-slate-900 dark:text-white">{t('verifiedCitizenVault', 'Verified Citizen Document Vault')}</h1>
          <p className="text-sm text-slate-600 dark:text-slate-400">
            {t('vaultDesc', 'Securely store official welfare documents. The Rule Engine automatically uses these to verify scheme eligibility.')}
          </p>
        </div>
        ...
        >
          ...
          <span>{t('launchOCR', 'Launch AI OCR Scanner')}</span>
        </button>
      </div>
                ...
                {existingDoc ? (
                  <div className="space-y-2 text-xs">
                    <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                      <span className="text-slate-500 block text-[10px]">{t('docNumber', 'Document Number')}</span>
                    ...
                    {existingDoc.ocrExtracted && (
                      <div className="p-2.5 rounded-lg bg-blue-50/50 dark:bg-slate-900/80 border border-blue-100 dark:border-slate-800 text-[11px] space-y-1">
                        <span className="font-bold text-blue-900 dark:text-blue-300 block">{t('ocrMetadata', 'OCR Extracted Metadata')}</span>
                        {existingDoc.ocrExtracted.fullName && <p>{t('name', 'Name')}: <strong>{existingDoc.ocrExtracted.fullName}</strong></p>}
                        {existingDoc.ocrExtracted.annualIncome && <p>{t('income', 'Income')}: <strong>₹{existingDoc.ocrExtracted.annualIncome.toLocaleString('en-IN')}</strong></p>}
                        <p className="text-[10px] text-slate-500">{t('confidence', 'Confidence')}: {existingDoc.ocrExtracted.confidenceScore}%</p>
                      </div>
                    )}
                  </div>
                ) : (
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {t('uploadOrScan', 'Upload or scan this document to unlock schemes requiring {t(docType, docType)} verification.').replace('{t(docType, docType)}', docType)}
                  </p>
                )}
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                {existingDoc ? (
                  <>
                    ...
                    >
                      <RefreshCw className="w-3 h-3" />{t('replaceRescan', 'Replace / Rescan')} </button>
                    <button
                  ...
                  >
                    <ScanLine className="w-4 h-4" />
                    <span>{t('scanWithOCR', 'Scan with AI OCR')}</span>
                  </button>
                )}
              </div>
            </div>
